...
for k = 1:n_opt_func
    ...
    for j = 1:n_obj_func
        ...
        for i = 1:num_experiments  
            ...
            % Adapt the sphere values from 800 to 1100 to be zero, as we don't have information for them
            sphere_SPD = zeros(1, length(WaveLenght)); 
            load(append("/calibration_tests/sphere_measurements", experiment_name(i),"_S.mat"));
            sphere_SPD(1:421) = data.Wrad_relative;
...
for j = 1:n_opt_func
    ...
    xtickangle(45);
    legend({'Sem calibração', 'MSE', 'WMSE', 'MAE', 'RMSE'}, 'Location', 'bestoutside');
...
Nome_grafico = "RGBW";
load(append("/calibration_tests/sensor_measurements",experiment,"-100_amostras.mat"));
file = append("/calibration_tests/sphere_measurements",experiment,"_S.txt")';
...
reconstrucao_semDifo = matrix_GSCM * sensor_values_no_diff';
...
minValue = min([min(reconstrucao_semDifo), min(SPD_MSE), min(SPD_WMSE), min(SPD_MAE), min(SPD_RMSE), min(sphere_SPD)]);
maxValue = max([max(reconstrucao_semDifo), max(SPD_MSE), max(SPD_WMSE), max(SPD_MAE), max(SPD_RMSE), max(sphere_SPD)]);
% Add colored rectangle to represent the spectrum colors
for i = 1:length(WaveLenght)-1
    x = [WaveLenght(i), WaveLenght(i+1), WaveLenght(i+1), WaveLenght(i)];
...
exportgraphics(f,append('saved_images/SPD_recontruido_metodos',experiment,'.pdf'),'ContentType','vector');
...
function SPD_reconstruida_norm = spectrum_restore(coeficientes_calibracao, sensor_values)
    ...
    if any(coeficientes_calibracao < lower_limits) || any(coeficientes_calibracao > upper_limits)
